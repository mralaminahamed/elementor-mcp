...
= 3.3.0 =
ACF / ACF PRO support.
* Added: 15 ACF tools that register only when Advanced Custom Fields (free or PRO) is active. Field values, field groups, and options pages: list-acf-field-groups, get-acf-field-group, list-acf-options-pages, get-acf-fields (reads), update-acf-fields, create-acf-field-group, update-acf-field-group (writes). Custom Post Types and taxonomies (ACF 6.1+): list-acf-post-types, get-acf-post-type, list-acf-taxonomies, get-acf-taxonomy (reads), create-acf-post-type, update-acf-post-type, create-acf-taxonomy, update-acf-taxonomy (writes). Reads are enabled by default; the 7 write tools ship disabled-by-default (opt in on the Tools tab).
...
* Added: ACF-managed Custom Post Types and taxonomies are registered as data through ACF (acf_import_post_type / acf_import_taxonomy) — no PHP is written or executed. Together with the field tools this is enough to build a full content structure end-to-end: CPT to taxonomy to field group to posts with values.
