...
> **These are REGISTERED counts** (without ACF; when Advanced Custom Fields free/Pro is active, **+15 ACF tools** register on top — 8 reads on by default, 7 writes off; the 8 CPT/taxonomy tools need ACF 6.1+, v3.3.0). **46 tools ship disabled-by-default** — SEO & Accessibility (**7**, Pro), Widget Builder (**8**, Pro), PHP Snippets / Sandbox (**6**, free), the **9** Plugins & Themes write tools, **1** `delete-media`, the **2** Users write tools, the **3** Filesystem write tools, the **3** Database write tools, and the **7** ACF write tools (env-gated). So the typical **active** surface is up to 46 smaller until a user enables them on the Tools tab (e.g. Pro + Elementor 4.0+ = **141** registered, **102** active by default). Verified reconciliation: 141 registered − 12 currently-off writes (SEO/WB/PHP-Snippets) − 3 filesystem writes − 3 database writes = 123 in the live `tools/list` when SEO/Widget-Builder/PHP-Snippets are manually enabled.
>
...
### ACF (Advanced Custom Fields) — 15 tools (v3.3.0)

Read/write ACF field values on posts and options pages, discover + author field groups, and register ACF-managed Custom Post Types and taxonomies over MCP — built on the public ACF API (`acf_get_field_groups`, `acf_get_fields`, `get_field`, `update_field`, `acf_import_field_group`, `acf_update_field_group`/`acf_update_field`, plus `acf_get_acf_post_types`/`acf_get_acf_taxonomies`/`acf_import_post_type`/`acf_import_taxonomy`/`acf_get_internal_post_type`/`acf_update_internal_post_type` for CPT/tax). The group **only registers when ACF (free or Pro) is active** (`EMCP_Tools_ACF_Abilities::acf_active()` gate in the registrar; every slug excluded from the drift guard via `EMCP_Tools_Admin::acf_tool_slugs()`); the **8 CPT/taxonomy tools register only on ACF 6.1+** (`cpt_tax_supported()` gate inside `register()`). Pro-only field features (options pages; repeater/flexible_content/gallery/clone field types) degrade gracefully on free ACF (`is_pro()` via `acf_get_setting('pro')`). Writes always resolve field **names → keys** before `update_field()` (by-name writes silently fail on targets with no stored value); flexible-content rows are validated against the field's layouts (`acf_fc_layout`). CPTs/taxonomies are registered **as data** through ACF (never `register_post_type` — no executable PHP), so the flow CPT → taxonomy → field group → posts-with-values is fully agent-driven. Deliberately conservative: **no delete tools**, fields can never be removed, a field's `name`/`key`/`type` — or a post type / taxonomy slug — can never change (`immutable_slug`), and code-registered (acf-json/PHP, `local` flag) groups are refused by `update-acf-field-group`; the CPT/tax read tools list only ACF-managed items. Field reads (`edit_posts`) and CPT/tax reads (`manage_options`) enabled by default; the 7 writes ship disabled-by-default (defaults markers v12 for the 3 field writes, v13 for the 4 CPT/tax writes). Admin category `wp_acf` (WordPress tab). Implemented in `includes/abilities/class-acf-abilities.php`; standalone public tests in `tests/` (`vendor/bin/phpunit -c tests/phpunit.xml` — runs without the private `pro/tests` submodule).
