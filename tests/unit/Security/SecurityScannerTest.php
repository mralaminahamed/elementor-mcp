...
class SecurityScannerTest extends TestCase {
	...
	private function scanner(): \EMCP_Tools_Security_Scanner {
		return new \EMCP_Tools_Security_Scanner();
	}

	private function finding( string $category, string $status, string $rec = 'fix it', string $label = 'L' ): array {
		return \EMCP_Tools_Security_Finding::make( 'id', $category, $label, $status, null, 'm', $rec );
	}
